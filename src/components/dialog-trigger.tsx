...
export function DialogComponent ( { buttonName, title }: Prop ) {
    ...
    const getAllSuppliers = async () => {
        try {
            const allSuppliers = await axiosInstanceClient.get( `${ process.env.NEXT_PUBLIC_BACKEND_URL }/Supplier` );
    ...
    const getAllItems = async () => {
        try {
            const allItems = await axiosInstanceClient.get( `${ process.env.NEXT_PUBLIC_BACKEND_URL }/Item` );
    ...
    async function onSubmit ( values: z.infer<typeof newPurchaseFormSchema> ) {
        try {
            if ( buttonName === 'New Purchase' ) {
                ...
                const newPurchaseResponse = await axiosInstanceClient.post( `${ process.env.NEXT_PUBLIC_BACKEND_URL }/Purchase`, purchaseValues );
                if ( newPurchaseResponse.status === 201 ) {
                    setOpen( false );
                }
            }
        } catch ( error ) {
                    ...
                    { buttonName === 'New Purchase' ? (
                        <Form { ...newPurchase }>
                            <form onSubmit={ newPurchase.handleSubmit( onSubmit ) } className="space-y-8">
                                            ...
                                            render={ ( { field: { onChange, value } } ) => (
                                                <div>
                                                    ...
                                                    >
                                                        ...
                                                        { items.filter( item => !selectedItems.includes( item.asin ) || item.asin === value ).map( ( item ) => (
                                ...
                                <div className='flex justify-around'>
                                    <div className=''>
                                        <Button type="button" onClick={ () => append( { name: '', quantity: 0, price: 0 } ) }>Add Item</Button>
                                    </div>
                                    <div className="">
