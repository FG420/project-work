...
export async function addPurchase ( purchase: any ) {
    try {
        const purchaseCall = await axiosInstanceServer.post(
            `${ process.env.NEXT_PUBLIC_BACKEND_URL }/Purchase`,
            purchase,
        );

        const purchasedItems = purchase.purchasedItems.map( ( item: { asin: any; quantity: any; price: any; } ) => ( {
            PurchaseID: purchaseCall.data.purchaseID,
            ASIN: item.asin,
        ...
        postPurchasedItems( purchasedItems );
...
export async function postPurchasedItems ( items: any[] ) {
    try {
        const promises = items.map( async ( item ) => {
            return await axiosInstanceServer.post(
